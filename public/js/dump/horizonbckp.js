...
function deformCylinder(verticesLength) {
    ...
    for ( var i = 0; i < verticesLength; i ++ ) {
        ...
        var noiseX = (1 + k * perlin.noise(i%2, i%5, i/17 * modulate) * Math.sin(modulate));
        var noiseZ = (1 + k * perlin.noise(i/13, i%11 , i%3 * modulate) * Math.cos(modulate)) ;

        var x = cylinderTerrainGeometry.vertices[i].x * noiseX;
        var y = cylinderTerrainGeometry.vertices[i].y;
        var z = cylinderTerrainGeometry.vertices[i].z * noiseZ;
        // cylinderTerrainGeometry.vertices[i] = new THREE.Vector3(x,y,z);
...
function animate() {
    ...
    for ( var i = 0; i < cylinderTerrainGeometry.vertices.length; i ++ ) {
        cylinderTerrainGeometry.vertices[i] = newCylinderVertices[i] ;
    }
